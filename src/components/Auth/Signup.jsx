...
import { initiateGoogleLogin, signinUsingGoogle, signupUsingEmail } from "../../services/operations/AuthAPI";
...
const Signup = () => {
    ...
    const handleSubmit = async (e, label) => {
        ...
        if (label === "email") {
            try {
                const response = await signupUsingEmail(formData.fullName, formData.email, formData.password);
                ...
                console.log("Error while sign up using email: ", error);
            }
			...
			try {
				...
				toast.error("Please check the credentials again");
                console.log("Error while sign up using google: ", error);
			}
        }
    };
